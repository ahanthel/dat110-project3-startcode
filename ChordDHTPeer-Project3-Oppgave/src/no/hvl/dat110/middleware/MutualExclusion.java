...
public class MutualExclusion
{
    ...
    public boolean doMutexRequest(Message message, byte[] updates) throws RemoteException
    {
        ...
        if (areAllMessagesReturned(messages.size()) && WANTS_TO_ENTER_CS)
        {
            // if yes, acquireLock
            node.acquireLock();
    ...
    private void multicastMessage(Message message, List<Message> activenodes) throws RemoteException
    {
        ...
        for (Message node : activenodes)
        {
            ...
            Util.getProcessStub(node).onMutexRequestReceived(message);
        }
    }
    ...
    public void doDecisionAlgorithm(Message message, List<Message> queue, int condition) throws RemoteException
    {
        switch (condition)
        {
            ...
            case 0:
            {
                ...
                NodeInterface stub = Util.getProcessStub(message);
            ...
            case 2:
            {
                ...
                if (clock.getClock() < message.getClock() ||
                        (clock.getClock() == message.getClock()) && node.getNodeID().compareTo(message.getNodeID()) > 0)
                {
                    queue.add(message);
                    break;
                }
                message.setAcknowledged(true);
                Util.getProcessStub(message).onMutexAcknowledgementReceived(message);
                break;
            }
    ...
    public void multicastReleaseLocks(Set<Message> activenodes) throws RemoteException
    {
        ...
        {
            // obtain a stub for each node from the registry
            NodeInterface stub = Util.getProcessStub(node);
            // call releaseLocks()
            stub.releaseLocks();
        }
    ...
    private boolean areAllMessagesReturned(int numvoters) throws RemoteException
    {
		...
		boolean same = queueack.stream().filter(q -> q.isAcknowledged()).count() == numvoters;
		// clear the queueack
		queueack.clear();
